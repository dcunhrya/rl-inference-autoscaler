...
_DEFAULT_MODEL = {"fcnet_hiddens": [256, 256], "fcnet_activation": "relu"}
...
def build_ppo_config(settings: TrainingSettings):
    """
    Build a Ray RLlib PPOConfig for AutoscalerEnv (new API stack).
    ...
    config = (
        ...
        .rl_module(model_config=dict(_DEFAULT_MODEL))
...
def validate_ppo_config(settings: TrainingSettings | None = None) -> dict[str, Any]:
    """Smoke-check that RLlib PPO config builds and validates without starting Ray."""
