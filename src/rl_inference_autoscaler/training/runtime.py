...
from rl_inference_autoscaler.training.config import DQNSettings, TrainingSettings
...
def _default_learner_metrics(learners: dict[str, Any]) -> dict[str, Any]:
    default = learners.get("default_policy") or learners.get("default_policy_id") or {}
    if not default and learners:
        default = next(iter(learners.values()), {})
...
def extract_ppo_metrics(result: dict[str, Any], iteration: int) -> dict[str, Any]:
    ...
    policy = _default_learner_metrics(learners)
    entropy = policy.get("entropy") or learners.get("entropy")
    return {
        "iteration": iteration,
...
def extract_dqn_metrics(result: dict[str, Any], iteration: int) -> dict[str, Any]:
    env_runners = result.get("env_runners") or {}
    learners = result.get("learners") or {}
    default_learner = _default_learner_metrics(learners)
